...
var interpreter_version = "1.2.1-MPI";
        ...
        getter.run(testRun, function (info) {
            ...
            if (testRun.currentStep().negated) {
                if (match) {
                    callback({
                        'success': false,
                        'error': new Error(getter.cmp ? getter.cmp + ' matches' : getter.name + ' is true')
                    });
                } else {
                ...
                if (match) {
                    ...
                    callback({
                        'success': false,
                        'error': new Error(getter.cmp ? getter.cmp + ' does not match' : getter.name + ' is false')
                    });
                }
            }
        });
            ...
            getter.run(testRun, function (info) {
                if (!info.error && !!((getter.cmp ? ("" + info.value) == testRun.p(getter.cmp) : info.value) ^ testRun.currentStep().negated)) {
                    ...
                    if (hr[0] + hr[1] * 1e-9 < testRun.script.timeoutSeconds) {
                        ...
                        testRun.do('takeScreenshot', [],
                            function (result) {
                            },
                            function (err, base64Image) {
                                var decodedImage = new Buffer(base64Image, 'base64');
                                fs.writeFile(argv.screenshotsPath + '/' + testRun.script.steps[0].text + '-' + testRun.name + '-step-' + (1 + testRun.stepIndex) + '-FAILED.png', decodedImage)
                            }
                        );
                ...
                function runStep() {
                    testRun.next(function (info) {
                        ...
                        if (testRun.hasNext()) {
                            ...
                            testRun.end(function (endInfo) {
                                runCallback({
                                    'success': testRun.success && endInfo.success,
                                    'error': testRun.lastError || endInfo.error
                                });
                            });
                        }
                    });
...
function loadData(dataConfig, dataSources, scriptPath) {
    ...
    if (dataSources) {
        var sources = dataSources.filter(function (ds) {
            return ds.name == configSource;
        });
        if (sources.length > 0) {
    ...
    var sources = defaultDataSources.filter(function (ds) {
        return ds.name == configSource;
    });
    if (sources.length == 0) {
...
if (listener) {
    listenerFactory = function (tr, listenerOptions) {
        return listener.getInterpreterListener(tr, listenerOptions, exports);
    };
} else {
...
if (numParallelRunners > 1 && !testRuns.every(function (tr) {
        return tr.quitDriverAfterUse;
    })) {
